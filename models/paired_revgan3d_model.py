...
class PairedRevGAN3dModel(BaseModel):
    ''' Paired 3D-RevGAN model '''
    def name(self):
        return 'PairedRevGAN3dModel'
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.netD_AB = networks3d.define_D(opt.input_nc + opt.output_nc, opt.ndf,
                                            opt.which_model_netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)
            self.netD_BA = networks3d.define_D(opt.input_nc + opt.output_nc, opt.ndf,
                                            opt.which_model_netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    def backward_G(self):
        ...
        if self.opt.grad_reg > 0.0:
            ...
            Lgrad = torch.cat([x.grad.view(-1) for x in self.netG.parameters()])
