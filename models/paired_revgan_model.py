...
class PairedRevGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--ode', action='store_true', help='use ode')
        if is_train:
    ...
    def initialize(self, opt):
        ...
        self.netG_core = networks.define_G_core(opt.input_nc, opt.output_nc,
                                                opt.ngf, opt.which_model_netG, opt.norm, not opt.no_dropout,
                                                opt.init_type, opt.init_gain, self.gpu_ids, invertible=True, ode=opt.ode, n_downsampling=opt.n_downsampling,
